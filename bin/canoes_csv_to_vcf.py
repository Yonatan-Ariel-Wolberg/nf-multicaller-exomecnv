...
def write_vcf_header(vcf_file, sample_file, fai_file, is_combined=False, individual_sample=None):
    ...
    if is_combined:
        ...
        vcf_file.write("\t" + "\t".join(read_sample_list(sample_file)) + "\n")
    else:
    ...
    if is_combined:
        ...
        for sample, mutations in mutations_by_sample.items():
            for mutation in mutations:
                key = (mutation['CHR'], mutation['INTERVAL'])
                ...
                # Write mutation to the VCF file
                vcf_file.write(f"chr{chr_num}\t{start}\t.\t{ref}\t{alt}\t.\t{filter_status}\t{info}\t{format_field}")

                for s_id in sample_list:
                    if s_id == mutation['SAMPLE']:
                        vcf_file.write(f"\t./.:{safe_float(mutation['Q_SOME'])}")
                    else:
                        vcf_file.write("\t./.:0")
                vcf_file.write("\n")
...
def process_canoes_data(input_file, sample_file, fai_file, output_dir, is_combined=False, log_file=None):
    """Process CANOES data and generate VCF files for each sample or combined."""
    if log_file:
        setup_logging(log_file)
    ...
    if mutation_list:
        ...
        if is_combined:
            combined_vcf_path = os.path.join(output_dir, "combined_CANOES_output.vcf")
...
def main():
    ...
    process_canoes_data(args.input_file, args.sample_file, args.fai_file, args.output_dir, is_combined=args.combined, log_file=args.log_file)
